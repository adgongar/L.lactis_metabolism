...
def L_Lactis_metabolism(time: float, conc0: np.array, parameters: list):
    ...
    v_nox = Nox_regu * (
        ...
        / (
            ...
            + (1 + (NAD / km_nox_nad) + (NAD / km_nox_nad)**2) - 1
        )
    )
...
def simulate_condition(args: list):
    ...
    acetoin_ext_final = X[-1, 12]
    butane_final = X[-1, 13]

    return lactate_final, acetate_final, acetoin_ext_final, butane_final
...
def Objetive_function(parameters_estimate: list, index_params: list, IC: list,
    ...
    for val in nis:
        ...
        results.append(simulate_condition(args_list))
    ...
    # Calculate error
    err_lactate = ((lactate_final - lactate_mean) ** 2) / lactate_var
    err_acetate = ((acetate_final - acetate_mean) ** 2) / acetate_var
...
def de_callback(xk, convergence):
    """
    callback function to plot convergence and register iterations
    Save (xk, f) for every generation.
    Return True if you want to stop optimization.

    """
    global f_out, progress, optimization_history, obj_args

    f = Objetive_function(xk, *obj_args)

    # memory register
    optimization_history.append(f)
    f_out.write('\t'.join(map(str, xk)) + f'\t{f}\n')
    f_out.flush()

    print(f'Generación {progress:3d} | f = {f:9.4g} | \
          conv = {convergence:.3g}')
    progress += 1
...
def main():
    ...
    3. If enabled, optimize selected parameters via basinhopping,
    """
    ...
    # Load parameters
    param_dict, bounds_dict = import_parameters('parametros_input_v13.csv')
    ...
    obj_args = (
        index_params,        # índices de los parámetros a ajustar
        IC,                  # condiciones iniciales
        t,                   # vector de tiempo
        parameters,          # lista completa de parámetros base
        lactate_mean, lactate_var,
        acetate_mean, acetate_var
        )
    ...
    if optimizar:
        ...
        result = differential_evolution(
            func=Objetive_function,
            bounds=bounds_guess,        # lista de tuplas (lower, upper)
            args=obj_args,              # ← la tupla de arriba
            strategy='best1bin',
            popsize=10,
            maxiter=2,
            tol=0.001,
            ...
            updating='deferred',
            workers=-1,
            callback=de_callback,       # mantiene el callback
            disp=False
        )

        t1 = time()
        print(f'The parameters values that optimize the model are: \
            {result}. El proceso ha tardado {t1 - t0}s')
